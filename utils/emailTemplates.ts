...
export function buildEmailTemplate({ title, messageHtml, buttonText, buttonUrl, footerText }: EmailConfig): string {
  const buttonHtml = buttonText && buttonUrl ? `
    <div style="text-align: center; margin: 32px 0;">
      <a href="${buttonUrl}" style="background-color: #0D9488; color: #ffffff; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; font-size: 16px;">
        ${buttonText}
      </a>
    </div>
  ` : '';
...
<head>
  ...
  <style>
    ...
    .highlight-box { background-color: #f0fdfa; border-left: 4px solid #0D9488; padding: 16px; border-radius: 4px; margin: 24px 0; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      ...
      <div class="content">
        ...
        ${messageHtml}
        ${buttonHtml}
        <p style="margin-top: 32px; font-size: 14px; color: #6b7280;">Best regards,<br>The Hourly Team</p>
      </div>
    </div>
    <div class="footer">
      ${footerText || '© ' + new Date().getFullYear() + ' Hourly. All rights reserved.'}
    </div>
  </div>
</body>
</html>
...
export function getWelcomeEmailHtml(): string {
  return buildEmailTemplate({
    title: "Welcome to Hourly! 🚀",
    messageHtml: `
      <p>Hi there,</p>
      <p>We are thrilled to have you join Hourly—the premier platform for micro-consulting and 1-on-1 expert sessions.</p>
    ...
    footerText: "You received this email because you recently signed up for Hourly."
  });
}
...
export function getForgotPasswordEmailHtml(): string {
  return buildEmailTemplate({
    ...
    messageHtml: `
      <p>Hi there,</p>
      <p>We received a request to reset the password for your Hourly account. If you didn't make this request, you can safely ignore this email.</p>
